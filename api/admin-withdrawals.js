...
export default async function handler(req, res) {
  try {
    ...
    /*
     * =========================================================
     * 2. IDENTIFICAR RESERVA DA EMPRESA
     *
     * A carteira TRON configurada em
     * USDTMZ_TRON_WALLET_ADDRESS é considerada a principal
     * carteira USDT da empresa.
     *
     * Não criamos saldo artificial.
     * =========================================================
     */
    ...
    for (const wallet of wallets) {
      ...
      /*
       * Para USDT/TRX, somente a carteira principal configurada
       * é tratada como carteira da empresa.
       */
      if (isMainCompanyWallet(wallet)) {
        companyWallets.push(wallet);

        if (asset === "USDT") {
          reserveUSDT += normalizeNumber(wallet.balance);
        }

        if (asset === "TRX") {
          reserveTRX += normalizeNumber(wallet.balance);
        }
      }

      /*
       * MZN é tratado como saldo interno da reserva quando
       * existir uma carteira/registro com asset MZN.
       *
       * Aqui não assumimos que MZN existe fisicamente em uma
       * carteira TRON.
       */
      if (
        asset === "MZN" &&
        ...
        reserveMZN += normalizeNumber(wallet.balance);
      }
    }
    ...
    /*
     * =========================================================
     * 4. CALCULAR RESERVA MZN PELO HISTÓRICO
     *
     * Se existir saldo MZN registrado em wallets, usamos esse
     * saldo como principal.
     *
     * O histórico fica disponível separadamente.
     * =========================================================
     */

    const reserveTransactions = transactions
      .filter((tx) => {
    ...
    /*
     * =========================================================
     * 7. FORMATAR RETIRADAS
     * =========================================================
     */

    const normalWithdrawals = withdrawals.map((item) => ({
      id: item.id,
      type: "WITHDRAWAL",
      source: "USER_WITHDRAWAL",

      withdrawal_id: item.withdrawal_id || null,
    ...
    /*
     * =========================================================
     * 8. FORMATAR TRANSFERÊNCIAS BINANCE
     * =========================================================
     */

    const binanceTransfers = orders.map((order) => ({
      id: `BINANCE-${order.id}`,
      ...
      amount: normalizeNumber(order.usdt_amount),
      amount_mzn: normalizeNumber(order.amount),
      usdt_amount: normalizeNumber(order.usdt_amount),

      rate: normalizeNumber(order.rate),
      ...
      tx_hash: order.blockchain_tx_hash || null,
    ...
    const history = [
      ...
      ...reserveTransactions.map((item) => ({
        ...item,
        history_type: "RESERVE_TRANSACTION"
      }))
    ]
      .sort((a, b) => {
    ...
    /*
     * =========================================================
     * 10. ESTADO DA RESERVA
     * =========================================================
     */

    const reservedUSDT = binanceTransfers
      .filter((item) => {
      ...
      .reduce(
        (total, item) =>
          total + normalizeNumber(item.usdt_amount),
        0
      );

    /*
     * O saldo disponível nunca pode ser negativo.
     *
     * Não criamos USDT.
     */
    ...
    return json(res, 200, {
      ...
      reserve: {
        ...
        company_wallet: {
          ...
          address: process.env.USDTMZ_TRON_WALLET_ADDRESS
            ? maskAddress(
                process.env.USDTMZ_TRON_WALLET_ADDRESS
              )
            : null,
      ...
      /*
       * Carteiras encontradas no banco.
       * Endereços são mascarados para o Admin.
       */
      wallets: wallets.map((wallet) => ({
        id: wallet.id,
        asset: normalizeAsset(wallet.asset),
        balance: normalizeNumber(wallet.balance),
        status: normalizeStatus(wallet.status),
        network: wallet.network || null,
        wallet_address: maskAddress(
      ...
      withdrawals: normalWithdrawals,

      normal_withdrawals: normalWithdrawals,
      ...
      counts: {
        ...
        withdrawals: normalWithdrawals.length,
        binance_transfers: binanceTransfers.length,
        reserve_transactions:
          reserveTransactions.length,

        pending_withdrawals: pendingWithdrawals,

        processing_binance: processingBinance,

        completed_binance: completedBinance,

        failed_binance: failedBinance
      },
    ...
    return json(res, 500, {
      ok: false,
      error: "Erro interno ao carregar a reserva.",
      message:
